...
uint32 version_map[] = {
//prg       //galaxy    //htc 
0x3000001d, 0xba845703, 0x51c92bd8,
0x3000005b, 0xe63bc050, 0x90380d3d,
0,
};

uint32 __convert_version(uint32 prg, uint32 vers) {
/*
	int i;
	for(i=0; version_map[i]!=0; i+=3){
		if(version_map[i] == prg && version_map[i+2]==vers)
			return version_map[i+1];
	}
*/
	return vers;
}
    ...
    char * proto)
{
	...
	vers = __convert_version(prog, vers);
